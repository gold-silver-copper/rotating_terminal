...
- an offscreen export path for rendering one full rotation to an image sequence
- a shell script that turns exported frames into an MP4 with `ffmpeg`
...
3. Encodes `00001.png`, `00002.png`, ... into an H.264 MP4.
...
Export frames and build the MP4:
...
- The export script expects `ffmpeg` to be available on your `PATH`.
- The export bin writes numbered PNGs, which makes them easy to re-encode into other formats later.
